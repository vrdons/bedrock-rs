...
impl<S: Signaling + 'static> NethernetListener<S> {
        ...
        tokio::spawn(async move {
            ...
            while let Some(signal) = signals.next().await {
                if signal.signal_type == SignalType::Offer {
                    if let Err(e) = Self::handle_offer(signal, &signaling, &incoming_tx).await {
    ...
    async fn handle_offer(
        ...
        // Create peer connection (no STUN servers needed for LAN)
        ...
        let offer = webrtc::peer_connection::sdp::session_description::RTCSessionDescription::offer(signal.data.clone())?;
        ...
        // Signal the answer
        let answer_signal = Signal::answer(
        ...
        let connection_id = signal.connection_id;
        tokio::spawn(async move {
            let mut signals = signaling_clone.signals();
            while let Some(sig) = signals.next().await {
                if sig.connection_id == connection_id && sig.signal_type == SignalType::Candidate {
